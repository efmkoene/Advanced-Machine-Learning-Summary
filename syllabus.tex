...
\usepackage{amsmath,amssymb,mathtools,booktabs,multirow,colortbl,url,float,epsdice}
...
\begin{spexample}[The curse of cats and dogs and dimensionality]
...
\begin{testexample}[Graphical example in 2-D]
...
\end{testexample}
\begin{spexample}[Illustrating neural networks as graphs]
...
\begin{spexample}[Support vectors]
...
\end{equation}
\vspace{-0.5cm}
\begin{testexample}[Non-linear transformation (1)]
...
\end{testexample}
\vspace{-0.15cm}
\begin{testexample}[Non-linear transformation (2)]
...
\begin{testexample}[Decision tree]
...
\end{equation}
\begin{testexample}[Ensemble methods]
...
\begin{testexample}[MLE of Gaussians]
...
\begin{testexample}[Naive Bayes model]
...
\begin{mymathbox}[ams align, title={MLE, Naive Bayes \& MAP}, colframe=blue!30!black, center title]
    \text{MLE:}\quad & \argmax_{\vec{\theta}} \mathbb{P}(\mathcal{Z}^\text{train}|\vec{\theta})\propto\argmax_{\vec{\theta}} \prod_{i=1}^N \mathbb{P}(y_i|x_i,x_{i+1},\dots,x_n,\vec{\theta}), \\
    \text{MAP:}\quad & \argmax_{\vec{\theta}}\mathbb{P}(\vec{\theta}|\mathcal{Z}^\text{train})\propto \argmax_{\vec{\theta}}\prod_{i=1}^N \mathbb{P}(y_i|x_i,x_{i+1},\dots,x_n,\vec{\theta})\mathbb{P}(\vec{\theta}),\\
    \text{Naive Bayes:}\quad & \argmax_{\vec{\theta}} \mathbb{P}(\mathcal{Z}^\text{train}|\vec{\theta})\propto\argmax_{\vec{\theta}} \prod_{i=1}^N \mathbb{P}(y_i|x_i,\vec{\theta})\mathbb{P}(\vec{\theta}).
\end{mymathbox}
...
\begin{spexample}[Ordinary least squares]
...
\begin{testexample}[Ordinary least squares vs. Bayesian regression]
...
We thus make no choice for a linear model $\vec{w}$ or transformation $\phi$. We simply assume \text{all} possible lines that we can draw are possible models, and we let the data constrain the type of models that actually work for our data!
\begin{testexample}[Gaussian process regression]
...
\begin{spexample}[Expectation maximization]
...
\end{spexample}
\begin{spexample}[Latent Dirichlet Process]
...
\end{spexample}
\begin{spexample}[Generative Adversarial Network]
...
\begin{testexample}[Typical bias-variance trade-off]
    ...
    \end{center}
\end{testexample}
\begin{spexample}[Setting of the bias-variance trade-off]
...
\begin{spexample}[Bias-variance parts]
...
\begin{testexample}[Rao-Cramer bound for Gaussians]

...
\begin{testexample}[Gaussian Mixture Model]
...
\begin{mymathbox}[ams align, title={MLE, Naive Bayes \& MAP}, colframe=blue!30!black, center title]
    \text{MLE:}\quad & \argmax_{\vec{w}} \mathbb{P}(\mathcal{Z}^\text{train}|\vec{w})=\argmax_{\vec{w}} \prod_{i=1}^N \mathbb{P}(y_i|x_i,x_{i+1},\dots,x_n,\vec{w}) \\
    \text{Naive Bayes:}\quad & \argmax_{\vec{w}} \mathbb{P}(\mathcal{Z}^\text{train}|\vec{w})=\argmax_{\vec{w}} \prod_{i=1}^N \mathbb{P}(y_i|x_i,\vec{w}) \\
    \text{MAP:}\quad & \argmax_{\vec{w}}\mathbb{P}(\vec{w}|\mathcal{Z}^\text{train})\propto \argmax_{\vec{w}}\prod_{i=1}^N \mathbb{P}(y_i|x_i,x_{i+1},\dots,x_n,\vec{w})\mathbb{P}(\vec{w}).
\end{mymathbox}
...
We'll do a quick run-down of three typical regression perspectives/algorithms.
